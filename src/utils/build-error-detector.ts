...
function parseDetailedBuildError(
  error: BuildError | string,
): BuildErrorDetails {
...
function sendErrorToParent(
  error: BuildError | string,
  additionalDetails: Partial<BuildErrorDetails> = {},
) {
  try {
    const errorMessage = error instanceof Error ? error.message : error;
...
if (typeof window !== "undefined") {
  ...
  window.addEventListener(
    ...
    true,
  ); // Use capture phase to catch errors before they're handled elsewhere
  ...
  window.__NEXT_BUILD_ERROR_LISTENER__ = function (
    errorData: BuildError | string,
  ) {
    try {
